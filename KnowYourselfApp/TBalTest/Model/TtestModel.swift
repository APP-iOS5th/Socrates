...
protocol TtestProtocol {
    var id: UUID { get set }
    var questionAsk: String { get set }
    var questions: [TtestList] { get set }
}

class Ttest: TtestProtocol {
    var id: UUID = UUID()
...
struct TtestStore {
    var quizListStore: [TtestProtocol] = [Ttest(questionAsk: "영화를 보고 나서 울고 있는 친구, 당신의 반응은?",
